...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        self.tab_widget.addTab(self.tab_global, "Global")
        self.tab_widget.addTab(self.tab_patterns, "Patterns")
        self.tab_widget.addTab(self.tab_samples, "Samples")
        self.tab_widget.addTab(self.tab_songs, "Songs")
    ...
    def _setup_actions(self):
        ...
        self.save_action = QAction(icons.icon("floppy-disk"), "Save", self)
        self.save_action.setShortcut("Ctrl+S")
        self.save_action.setToolTip("Save (Ctrl+S)")
        self.save_action.triggered.connect(self.save_file)

        self.save_as_action = QAction(icons.icon("floppy-disk"), "Save As...", self)
        self.save_as_action.setShortcut("Ctrl+Shift+S")
        self.save_as_action.triggered.connect(self.save_file_as)

        self.quit_action = QAction(icons.icon("right-from-bracket"), "Quit", self)
        self.quit_action.setShortcut("Ctrl+Q")
        self.quit_action.triggered.connect(self.close)

        self.import_samples_action = QAction(icons.icon("file-import"), "Import Sample(s)...", self)
        self.import_samples_action.setShortcut("Ctrl+I")
        self.import_samples_action.setToolTip("Import WAV sample(s) (Ctrl+I)")
        self.import_samples_action.triggered.connect(self._import_samples)

        self.import_as_mono_action = QAction("Import as Mono", self)
        ...
        self.delete_unused_samples_action = QAction(icons.icon("trash"), "Delete Unused Samples...", self)
        self.delete_unused_samples_action.triggered.connect(self._delete_unused_samples)

        self.about_action = QAction(icons.icon("circle-info"), "About", self)
        self.about_action.triggered.connect(self._show_about)

        self.send_debug_data_action = QAction("Sende Debug Daten", self)
    ...
    def _setup_dock(self):
        ...
        pattern_browser_dock = QDockWidget("Pattern Browser", self)
    ...
    def _load_esx_file(self, path: str):
        self._set_busy(True)
        progress = _make_busy_dialog(self, "Loading ESX file...")
    ...
    def _on_file_loaded(self, esx: EsxFile, path: str, progress):
        ...
        self.setWindowTitle(f"Open Electribe Editor - {os.path.basename(path)}")
    ...
    def _on_debug_upload_finished(self, path: str):
        self.statusBar().showMessage("Debug-Daten gesendet", 3000)

    def _on_debug_upload_error(self, error: str):
        report_error("debug_upload", Exception(error), self._debug_upload_thread.filepath)
        self.statusBar().showMessage(f"Debug-Upload fehlgeschlagen: {error}", 5000)
    ...
    def _do_save(self, path: str, is_save_as: bool = False):
        ...
        self._set_busy(True)
        progress = _make_busy_dialog(self, "Saving ESX file...")
    ...
    def _show_about(self):
        QMessageBox.information(
            self, "About",
            "Open Electribe Editor (Python)\n\n"
            "A Python/PyQt6 port of the open-electribe-editor.\n"
            "Supports Korg ESX-1 .esx files.\n\n"
            "Original Java version by SKRATCHDOT.COM"
        )
